function onSubmit(e) {
  ...
  try{
    ...
    MailApp.sendEmail(toEmail, subject, "", {htmlBody: JSON.stringify(vals), name: "GT RHA", replyTo: hcoEmail, });
    MailApp.sendEmail(hcoEmail, subject, "", {htmlBody: JSON.stringify(vals), name: "GT RHA", replyTo: hcoEmail, });
  }
  catch (error){
    // send what you can to the hco and it
    MailApp.sendEmail({to:hcoEmail, cc:"it@rha.gatech.edu", subject:"HC Error", htmlBody:error + JSON.stringify(vals)});
  }
}
